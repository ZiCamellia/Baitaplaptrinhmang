...
DWORD WINAPI ClientThread(LPVOID lpParam)
{
    ...
    int ret;
    char buf[256];
    char user[32], pass[32], tmp[32], tempf[32];
    // Xu ly dang nhap
    while (1)
    {
        ...
        else
        {
            ...
            while (!feof(f))
            {
                ...
                char userf[32], passf[32], tempflocal[32];
                sscanf(line, "%s %s %s", userf, passf, tempflocal);
                strcpy(tempf, tempflocal);
